...
// ⚠️ UPDATED: Changed edgeJsIntegration to vbnetExecutable
export interface ReportHealthResponse {
  status: 'ok' | 'error';
  reportGenerator: string;
  vbnetExecutable: 'available' | 'not available' | 'unknown';  // Changed from edgeJsIntegration
  directories: {
    output: {
      path: string;
      exists: boolean;
 ...
 * @param params - Query parameters containing county, dateFrom, dateTo
 * @returns Promise with NSF performance data
...
export const getNsfPerformance = async (
  ...
  try {
    ...
    const response = await axios.get<NsfPerformanceResponse>(
      `${API_BASE_URL}/nsf-performance`,
      {
        params: {
 ...
 * @param params - Query parameters containing submid, dateFrom, dateTo
 * @returns Promise with lab details data
 * @throws ErrorResponse if request fails
 */
export const getNsfPerformanceLabDetails = async (
  ...
  try {
    ...
    const response = await axios.get<LabDetailsResponse>(
      `${API_BASE_URL}/nsf-performance/lab-details`,
      {
        params: {
 ...
 * @param params - Report generation parameters (submid, dateFrom, dateTo)
 * @returns Promise with PDF blob generated by Crystal Reports
 * @throws ReportErrorResponse if generation fails
 */
export const generateNsfReport = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${API_BASE_URL}/nsf-performance/generate-report`,
      {
        params: {
          submid: params.submid,
          dateFrom: params.dateFrom,
    ...
    if (axios.isAxiosError(error)) {
      // If error response is JSON (Blob), parse it
      ...
      if (error.code === 'ECONNABORTED') {
        throw new Error('Crystal Report generation timed out. The report may be too large or the server is busy. Please try again.');
      }
      
      // Handle network errors
      if (error.code === 'ERR_NETWORK') {
        throw new Error('Network error: Unable to connect to Crystal Reports server. Please check your connection.');
      }
      
      // Handle server errors
      if (error.response?.status === 500) {
        throw new Error('Crystal Reports generation failed on the server. Please contact support.');
      }
      
      throw new Error(error.response?.data?.error || error.message || 'Failed to generate Crystal Report');
    }
    throw error;
  }
};
...
/**
 * Helper function to download PDF blob with proper filename
...
export const downloadReportBlob = (blob: Blob, params: GenerateReportParams): void => {
  try {
    ...
    // Generate filename: nsf_performance_{submid}_{dateFrom}_{dateTo}.pdf
...
/**
 * Helper function to open PDF blob in new tab
 * @param blob - PDF blob from generateNsfReport
 */
export const openReportInNewTab = (blob: Blob): void => {
  try {
    ...
    // Clean up URL after a delay (window might still be loading)
 ...
 * @returns Promise with list of report files and metadata
 */
export const listReports = async (): Promise<ListReportsResponse> => {
  try {
    ...
    const response = await axios.get<ListReportsResponse>(
      `${API_BASE_URL}/nsf-performance/reports`,
...
export const getReportByFilename = async (filename: string): Promise<Blob> => {
  try {
    ...
    const response = await axios.get(
      `${API_BASE_URL}/nsf-performance/reports/${encodeURIComponent(filename)}`,
      {
        responseType: 'blob',
...
/**
 * Helper function to download a specific Crystal Report file
 ...
 * @returns Promise with system health information including VB.NET executable status
 */
export const getReportSystemHealth = async (): Promise<ReportHealthResponse> => {
  try {
    console.log('🏥 Checking Crystal Reports system health...');
    
    const response = await axios.get<ReportHealthResponse>(
      `${API_BASE_URL}/nsf-performance/report-health`,
      { timeout: 10000 } // 10 seconds
    );
    ...
    console.log('✅ Health check complete:', {
      ...
      vbnetExe: response.data.vbnetExecutable,  // Changed from edgeJs
      templateExists: response.data.template.exists,
    });
    
    // Warn if Crystal Reports is not available
...
/**
 * Validate Crystal Reports setup before generating reports (VB.NET Executable)
...
export const validateCrystalReportsSetup = async (): Promise<boolean> => {
  try {
    ...
    // ⚠️ UPDATED: Check vbnetExecutable instead of edgeJsIntegration
    const isValid = 
      health.status === 'ok' &&
