...
const COLORS = ["#3b82f6","#22c55e","#f97316","#ef4444","#a855f7","#eab308","#06b6d4","#ec4899","#84cc16","#f59e0b"];
...
const CHART_ID = "job-order-by-status-chart";
...
const renderActiveShape = (props: any) => {
  ...
  return (
    <g>
      ...
      />
      <text x={cx} y={cy - 16} textAnchor="middle" fill={fill} fontSize={12} fontWeight="bold">
        {payload.label}
      </text>
...
export const ITJobOrderByStatusChart: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    for (const jo of filtered) {
      counts[jo.status] = (counts[jo.status] ?? 0) + 1;
    }
  ...
  const handleExport = async (format: "png" | "svg" | "excel") => {
    ...
    await downloadChart({
      elementId: CHART_ID,
      filename: `JobOrders_ByStatus_${year}-${String(month).padStart(2, "0")}`,
      format,
  ...
  return (
    <div className="flex flex-col rounded-2xl shadow-lg overflow-hidden bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 h-[420px]">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b flex-shrink-0 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700">
        <div>
          <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">Job Orders by Status</h3>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Tooltip
                ...
                formatter={(value: any, _: any, entry: any) => [value, entry.payload.label]}
              />
              <Legend verticalAlign="bottom" height={36} wrapperStyle={{ fontSize: "11px" }}
                formatter={(_: any, entry: any) => entry.payload.label}
              />
              <Pie
                data={chartData} dataKey="count" nameKey="label"
