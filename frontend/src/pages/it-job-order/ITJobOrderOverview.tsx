...
const STATUS_COLORS: Record<string, string> = {
  ...
  queued:           'bg-indigo-100 text-indigo-700 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800',
  ...
  resolved:         'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800',
  closed:           'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-700/50 dark:text-slate-400 dark:border-slate-600',
...
const STAT_CARD_DARK: Record<string, string> = {
...
function RelativeTime({ date }: { date: Date }) {
  ...
  return (
    <span title={fullDate} className="flex items-center gap-1 cursor-default text-slate-400 dark:text-slate-500">
...
function RejectModal({ workOrderNo, onConfirm, onCancel, isLoading }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[10001] flex items-center justify-center p-4">
      <style dangerouslySetInnerHTML={{ __html: MODAL_KEYFRAME }} />
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl w-full max-w-md p-6" style={MODAL_STYLE}>
        <h3 className="text-lg font-bold text-slate-900 dark:text-gray-100 mb-1">Reject Job Order</h3>
...
function AssignTechModal({ workOrderNo, onConfirm, onCancel, isLoading }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[10001] flex items-center justify-center p-4">
      <style dangerouslySetInnerHTML={{ __html: MODAL_KEYFRAME }} />
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl w-full max-w-md p-6" style={MODAL_STYLE}>
        <h3 className="text-lg font-bold text-slate-900 dark:text-gray-100 mb-1">Assign IT Officer</h3>
        <p className="text-sm text-slate-500 dark:text-gray-400 mb-4">
          Assigning <span className="font-mono font-bold text-slate-700 dark:text-gray-200">{workOrderNo}</span> to an IT Officer.
        </p>
        <select value={selectedTech} onChange={(e) => setSelectedTech(Number(e.target.value))}
          className="w-full px-3 py-2 text-sm border border-slate-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4 bg-white dark:bg-gray-800 text-slate-900 dark:text-gray-100">
...
function ResolveModal({ workOrderNo, onConfirm, onCancel, isLoading }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[10001] flex items-center justify-center p-4">
      <style dangerouslySetInnerHTML={{ __html: MODAL_KEYFRAME }} />
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto" style={MODAL_STYLE}>
        <h3 className="text-lg font-bold text-slate-900 dark:text-gray-100 mb-1">Resolve Job Order</h3>
        <p className="text-sm text-slate-500 dark:text-gray-400 mb-4">
          Resolving <span className="font-mono font-bold text-slate-700 dark:text-gray-200">{workOrderNo}</span>. Please provide details.
        </p>
...
export function ITJobOrderOverview() {
  ...
  const handleResolveConfirm = (resolveData: { action_taken: string; resolution_notes?: string }) => {
    ...
    resolveMutation.mutate({ id: resolveTarget.id, data: resolveData }, { onSuccess: () => setResolveTarget(null) });
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-gray-400 font-medium">Loading job orders...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-gray-950 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-gray-100 mb-2">Error Loading Job Orders</h2>
          <p className="text-slate-600 dark:text-gray-400 mb-6">{error.message}</p>
  ...
  const headerConfig = {
    troubleshooter: {
      icon: '🛠️', title: 'IT Job Order Management',
    ...
    approver: {
      icon: '✅', title: `${perms.approvableDept} Dept Approvals`,
      badge: `${user?.position}`,
      badgeColor: 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800',
    },
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 transition-colors">
      <div className="max-w-7xl mx-auto p-6">
        ...
        {perms.isTroubleshooter && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
            {statCards.map(({ label, status, icon, color, count }) => (
              <StatCard
                key={status} label={label} status={status} icon={icon} color={color}
                count={count} isLoading={isLoadingStats}
        ...
        {/* ── Filters ── */}
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 p-4 mb-5">
          <div className={`grid gap-3 ${perms.isTroubleshooter ? 'grid-cols-2 md:grid-cols-5' : 'grid-cols-2 md:grid-cols-4'}`}>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 dark:text-gray-400 mb-1.5">Priority</label>
              <select
        ...
        {/* ── Ticket list ── */}
        {tickets.length === 0 ? (
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 p-14 text-center">
            ...
            <p className="text-slate-500 dark:text-gray-400 text-sm mb-6 max-w-sm mx-auto">
              {perms.isApprover
...
function TicketCard({
  ticket, perms, onCardClick, onApprove, onReject,
  onAssign, onStart, onResolve, onClose, isLoading,
}: {
  ticket: Ticket;
  ...
  return (
    <div
      className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 p-5 hover:shadow-md hover:border-blue-200 dark:hover:border-blue-800 transition-all cursor-pointer group"
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-gray-400">
            {(perms.isTroubleshooter || perms.isApprover) && (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  <span className="text-slate-400 dark:text-gray-500">From </span>
                  <span className="font-semibold text-slate-700 dark:text-gray-200">{ticket.requester.name}</span>
                </span>
              </div>
            )}
            ...
            {ticket.assignee && (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  <span className="text-slate-400 dark:text-gray-500">Tech: </span>
                  <span className="font-semibold text-slate-700 dark:text-gray-200">{ticket.assignee.name}</span>
                </span>
              </div>
            )}
