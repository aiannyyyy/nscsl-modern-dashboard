...
} = require('../../utils/itJobOrderNotifier');
...
exports.createJobOrder = async (req, res) => {
    ...
    try {
        ...
        const {
            ...
            priority = 'medium', department, location,
            asset_id, estimated_hours, tags
        } = req.body;
...
exports.getAllJobOrders = async (req, res) => {
    try {
        ...
        const {
            status, priority, department, tech_id, requester_id,
        ...
        const joinClause = `FROM it_job_order jo
        ...
        const [countResult] = await mysqlPool.query(
            `SELECT COUNT(*) as total ${joinClause} ${whereClause}`, params
        );
    ...
    try {
        ...
        const [rows] = await mysqlPool.query(
            `SELECT jo.*, tech.name as tech_name, tech.username as tech_username
             ...
             LEFT JOIN user tech ON jo.tech_id = tech.user_id
             WHERE jo.requester_id = ?
               AND jo.status IN ('pending_approval','approved','queued','assigned','in_progress','on_hold')
             ORDER BY
               CASE jo.priority
...
exports.updateJobOrder = async (req, res) => {
    ...
    try {
        ...
        const allowedFields = [
            ...
            'tech_id','action_taken','reason','resolution_notes',
            'estimated_hours','actual_hours','tags','location','asset_id','due_date'
        ];
        ...
        for (const [key, value] of Object.entries(req.body)) {
            if (allowedFields.includes(key) && value !== undefined) {
                ...
                if (current[key] !== value) {
                    await logHistory(connection, id, userId, 'updated', key, String(current[key]), String(value));
                    if (key === 'status') newStatus = value;
                }
            }
        }
        ...
        if (newStatus && current.requester_id) {
            notifyRequesterOnStatusUpdate({
                requesterId: current.requester_id,
                jobOrderId:  id,
                workOrderNo: current.work_order_no,
                newStatus,
                createdBy:   userName
            }).catch(err => console.error('❌ Notify status update error:', err.message));
        }
...
exports.deleteJobOrder = async (req, res) => {
    ...
    try {
        ...
        await logHistory(connection, id, userId, 'cancelled', 'status', jobOrder.status, 'cancelled');
        await connection.commit();

        // ✅ Notify requester their order was cancelled (only if someone else cancelled it)
        if (jobOrder.requester_id && jobOrder.requester_id !== userId) {
            notifyRequesterOnStatusUpdate({
                requesterId: jobOrder.requester_id,
                jobOrderId:  id,
                workOrderNo: jobOrder.work_order_no,
                newStatus:   'cancelled',
                createdBy:   userName
            }).catch(err => console.error('❌ Notify cancel error:', err.message));
        }
...
exports.approveJobOrder = async (req, res) => {
    ...
    try {
        ...
        // ✅ Notify requester after commit
        notifyRequesterOnApprove({
            requesterId: jobOrder.requester_id,
            jobOrderId:  id,
            workOrderNo: jobOrder.work_order_no,
            createdBy:   approverName
        }).catch(err => console.error('❌ Notify approve error:', err.message));

        // ✅ Notify all IT officers that a new order is in the queue
        notifyITOfficersOnQueue({
            jobOrderId:  id,
            workOrderNo: jobOrder.work_order_no,
...
exports.rejectJobOrder = async (req, res) => {
    ...
    try {
        ...
        notifyRequesterOnReject({
            ...
            reason,
            createdBy:   approverName
        }).catch(err => console.error('❌ Notify reject error:', err.message));
...
exports.assignJobOrder = async (req, res) => {
    ...
    try {
        ...
        const assignerName = req.user?.name || 'Manager';
        ...
        const [jobOrderRows] = await connection.query('SELECT * FROM it_job_order WHERE id = ?', [id]);
        const jobOrder = jobOrderRows[0];
        ...
        }).catch(err => console.error('❌ Notify tech (assign) error:', err.message));

        // ✅ Also notify requester that a tech has been assigned
        notifyRequesterOnStatusUpdate({
            requesterId: jobOrder.requester_id,
            jobOrderId:  id,
            workOrderNo: jobOrder.work_order_no,
            newStatus:   'assigned',
            createdBy:   assignerName
        }).catch(err => console.error('❌ Notify requester (assign) error:', err.message));
...
exports.startJobOrder = async (req, res) => {
    ...
    try {
        ...
        const techName  = req.user?.name || 'Technician';
        ...
        if (rows.length === 0) {
            return res.status(400).json({ success: false, message: 'Job order not found or not in assigned status' });
        }
        ...
        notifyRequesterOnStatusUpdate({
            ...
            createdBy:   techName
        }).catch(err => console.error('❌ Notify start error:', err.message));
...
exports.resolveJobOrder = async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0) {
            return res.status(400).json({ success: false, message: 'Job order not found or not in progress' });
        }
        ...
        if (jobOrder.started_at) {
            const startedAt = new Date(jobOrder.started_at);
        ...
        notifyRequesterOnResolve({
            ...
            workOrderNo: jobOrder.work_order_no,
            createdBy:   techName
        }).catch(err => console.error('❌ Notify resolve error:', err.message));
...
exports.closeJobOrder = async (req, res) => {
    ...
    try {
        ...
        await connection.query(
            `UPDATE it_job_order SET status = 'closed', closed_at = NOW(), closed_by_id = ? WHERE id = ?`,
            [userId, id]
        );
        await logHistory(connection, id, userId, 'closed', 'status', 'resolved', 'closed');
        await connection.commit();

        // ✅ Notify requester (only if someone else closed it, e.g. an admin)
        if (jobOrder.requester_id && jobOrder.requester_id !== userId) {
            notifyRequesterOnStatusUpdate({
                requesterId: jobOrder.requester_id,
                jobOrderId:  id,
                ...
                newStatus:   'closed',
                createdBy:   userName
            }).catch(err => console.error('❌ Notify close error:', err.message));
        }
...
exports.uploadAttachment = [
    ...
    async (req, res) => {
        ...
        try {
            ...
            const [result] = await connection.query(
                `INSERT INTO job_order_attachments (job_order_id, file_name, file_path, file_size, mime_type, uploaded_by)
                 VALUES (?, ?, ?, ?, ?, ?)`,
